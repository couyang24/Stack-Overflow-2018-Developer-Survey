...
by_country <- survey_results_public %>% select(Country) %>% filter(!is.na(Country)) %>% count(Country)
by_country$iso3 <- countrycode(by_country$Country, 'country.name', 'iso3c')
 


p_by_country <- highchart() %>% 
  hc_add_series_map(worldgeojson, by_country, value = "n", joinBy = "iso3") %>% 
  hc_colorAxis(stops = color_stops()) %>% 
  hc_legend(enabled = TRUE) %>%  
  hc_mapNavigation(enabled = TRUE) %>%
  hc_title(text = "Respondent by Country")  %>%
  hc_tooltip(useHTML = TRUE, headerFormat = "",
             pointFormat = "Country: {point.Country} Total Respondent: {point.n}") %>%  
...
survey_results_public$EducationTypes %>% table() 





survey_results_public2 <- survey_results_public %>% 
  # select(EducationTypes) %>% 
  mutate(EducationTypes = strsplit(as.character(EducationTypes), ";"))  %>%
  unnest(EducationTypes)
